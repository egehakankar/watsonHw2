...
found , moves1, w = beamer(S1, goal, goalLocs)
trace_states(moves1)
print(f"total moves: { len(moves1)}, W: {w}")
found , moves2, w = beamer(S2, goal, goalLocs)
trace_states(moves2)
print(f"total moves: { len(moves2)}, W: {w}")
found , moves3, w = beamer(S3, goal, goalLocs)
trace_states(moves3) #moves array contains all the states that lead upto the solution
print(f"total moves: { len(moves3)}, W: {w}")
...
i = 1
for move in moves1:
    move_g = printPuzzle(move, ("Initial State S1: ")) if i == 1 else printPuzzle(move, ("S1 move: "+ str(i)))
...
i = 1
for move in moves2:
    move_g = printPuzzle(move, ("Initial State S2: ")) if i == 1 else printPuzzle(move, ("S2 move: "+ str(i)))
    allPlots.append(move_g)
    with open('p_graph.html', 'a') as f:
        f.write(move_g.to_html(full_html=False, include_plotlyjs='cdn'))
    i = i + 1
...
i = 1
for move in moves3:
    move_g = printPuzzle(move, ("Initial State S3: ")) if i == 1 else printPuzzle(move, ("S3 move: "+ str(i)))
    allPlots.append(move_g)
    with open('p_graph.html', 'a') as f:
        f.write(move_g.to_html(full_html=False, include_plotlyjs='cdn'))
    i = i + 1
